...
3. `channel-list` — **generic "👥 Community" topbar** (no tenant name). Community/tenant **strip** below it: leading personal-chat icon (no label, B2C-style DM mode switcher), then a vertical divider, then the B2B community icons (Acme · Globex · Labs) with a short active-indicator underline under the selected one, then a dashed `+Join`. Below the strip sits an **active-tenant header** (big name + `›` caret + `🌐 General · N members` subtitle) with a trailing search icon. Then the pinned `announcement` row, then "Joined groups (N)" with domain sub-sections for the active community. Rich rows show a colored group avatar, name, relative time, last-message preview, and unread badge; tenant switching persists via `localStorage` under `kchat-b2b-tenant`.
